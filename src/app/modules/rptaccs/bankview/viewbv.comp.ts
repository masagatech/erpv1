...
export class BankViewReports implements OnInit, OnDestroy {
    ...
    getAPARDropDown(row) {
        ...
        that._rptservice.getBankView({
            ...
            try {
                that.fliterAPARDT = data.data[0]._apartype === null ? [] : data.data[0]._apartype;
                that.fliterBankDT = data.data[0]._bank === null ? [] : data.data[0]._bank;
                that.fliterBankTypeDT = data.data[0]._banktype === null ? [] : data.data[0]._banktype;
