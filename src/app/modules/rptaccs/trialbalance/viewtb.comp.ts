...
@Component({
    templateUrl: 'viewtb.comp.html',
    providers: [ReportsService, CommonService]
})

export class TrialBalance implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        that.setActionButtons.setTitle("Trial Balance");
    ...
    fillCustTypeDDL() {
        this.custTypeDT.push({ "key": "summary", "val": "Summary" });
        this.custTypeDT.push({ "key": "details", "val": "Details" });
    }

    // Get Trial Balance Grid

    getTrialBalance(from: number, to: number) {
        var that = this;
        that.trialbalanceDT = [];
        ...
        that._rptservice.getTrialBalance({
            "flag": that.custtype, "cmpid": that.loginUser.cmpid, "fy": that.loginUser.fy, "uid": that.loginUser.uid,
            "frmdt": frmdt.length !== 0 ? frmdt : null, "todt": todt.length !== 0 ? todt : null,
            "from": from, "to": to
        }).subscribe(tbr => {
            that.totalRecords = tbr.data[1][0].recordstotal;
            that.trialbalanceDT = tbr.data[0];
            that.gridTotal.DrAmtTotal += parseFloat(tbr.data[1][0]._totdramt);
            that.gridTotal.CrAmtTotal += parseFloat(tbr.data[1][0]._totcramt);
        }, err => {
    ...
    loadTBCGrid(event: LazyLoadEvent) {
        this.getTrialBalance(event.first, (event.first + event.rows));
    }

    filterTBReport(dt: DataTable) {
        dt.reset();
    }
    ...
    TotalAmount() {
        ...
        for (var i = 0; i < this.trialbalanceDT.length; i++) {
            var items = this.trialbalanceDT[i];
