...
export class CalendarComp implements OnInit {
    ...
    initialize(loginUser) {
        ...
        this.docdate.slotChar = loginUser._globsettings.dateformat.replace(/y/g, "yy");
        this.docdate.placeholder = loginUser._globsettings.dateformat.replace(/y/g, "yy");

        setTimeout(function () {
            $("[name='" + that.name + "']").datepicker({
                dateFormat: that.loginUser._globsettings.dateformat,
