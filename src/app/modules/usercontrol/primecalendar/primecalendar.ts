import {
    NgModule, Component, ElementRef, AfterViewInit, OnDestroy, OnInit, Input, Output, EventEmitter,
    SimpleChange, forwardRef, Renderer, ViewChild, trigger, state, style, transition, animate
} from '@angular/core';
import { CommonModule } from '@angular/common';
import { ButtonModule, InputMaskModule } from 'primeng/primeng';
import { DomHandler } from '../dom/domhandler';
import { FormsModule, AbstractControl, NG_VALUE_ACCESSOR, NG_VALIDATORS, ControlValueAccessor } from '@angular/forms';
    ...
    template: `
        <span [ngClass]="{'ui-calendar':true,'ui-calendar-w-btn':showIcon}" [ngStyle]="style" [class]="styleClass">
            <template [ngIf]="!inline">
                  <p-inputMask  #inputfield [attr.required]="required" [(ngModel)]="inputFieldValue"  mask="99/99/9999"
                   (focus)="onInputFocus(inputfield, $event)" (keydown)="onInputKeydown($event)" (click)="closeOverlay=false" (blur)="onInputBlur($event)"
                    ...
                    ></p-inputMask><button type="button" [icon]="icon" pButton *ngIf="showIcon" (click)="onButtonClick($event,inputfield)"
            ...
            <div #datepicker class="ui-datepicker ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" [ngClass]="{'ui-datepicker-inline':inline,'ui-shadow':!inline,'ui-state-disabled':disabled,'ui-datepicker-timeonly':timeOnly}" 
                ...
                <div class="ui-datepicker-header ui-widget-header ui-helper-clearfix ui-corner-all" *ngIf="!timeOnly && (overlayVisible || inline)">
                    <content select="p-header"></content>
                ...
                <content select="p-footer"></content>
            </div>
        </span>
    `,
    animations: [
...
export class PrimeCalendar implements AfterViewInit, OnInit, OnDestroy, ControlValueAccessor {
...
@NgModule({
    imports: [CommonModule, FormsModule, ButtonModule, InputMaskModule],
    exports: [PrimeCalendar, ButtonModule, InputMaskModule],
